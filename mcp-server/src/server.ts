...
5. For continuous price updates (API key): watch_product / list_watchlist, or pass watch=true on track_product. Creating a price alert also watches the product.
6. Price alerts: create_price_alert / list_price_alerts / get_price_alert / update_price_alert / delete_price_alert. These require a Pricewatcha API key (pwk_live_...). You can notify on any drop or rise without a numeric threshold (notify_on_drop / notify_on_rise), and optionally add min/max thresholds.
...
Catalog reads need no API key. Alert and watchlist tools require api_key. Prices are informational; merchant sites are authoritative.
...
export function createServer(): McpServer {
  ...
  registerWatchProduct(server);
  registerUnwatchProduct(server);
  registerListWatchlist(server);
  registerGetWatchStatus(server);
